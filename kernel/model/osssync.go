...
func getOssUploadToken(filename, cloudDirPath string, length int64) (ret string, err error) {
	...
	req.SetBody(map[string]interface{}{
		...
		"name":    filename,
		"length":  length})
	resp, err := req.Post(util.LocalServer + "/apis/siyuan/data/getSiYuanFileUploadToken?uid=" + Conf.User.UserId)
	if nil != err {
...
func getCloudSyncVer(cloudDir string) (cloudSyncVer int64, err error) {
	...
	resp, err := request.
		...
		Post(util.LocalServer + "/apis/siyuan/data/getSiYuanWorkspaceSyncVer?uid=" + Conf.User.UserId)
	if nil != err {
...
func getCloudFileListOSS(cloudDirPath string) (ret map[string]*CloudIndex, err error) {
	...
	resp, err := request.
		...
		Post(util.LocalServer + "/apis/siyuan/data/getSiYuanFileListURL?uid=" + Conf.User.UserId)
	if nil != err {
